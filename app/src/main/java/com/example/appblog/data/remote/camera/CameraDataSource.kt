...
class CameraDataSource {
    ...
    suspend fun uploadPhoto(imageBitmap: Bitmap, description: String) {
        ...
        val imageRef = FirebaseStorage.getInstance().reference.child("${user?.uid}/publicaciones/$randomName")
        ...
        user?.let {
            it.displayName?.let { displayName ->
                FirebaseFirestore.getInstance().collection("publicaciones").add(Post(
